...
`adminLive()` hit `/admin/dashboard/live` every 15s. That endpoint rebuilt the full roster summary **twice** (`dashboardSummary` + `departmentSummaries`) and loaded 50 employee rows, then the browser only stored `serverTime` (which was never displayed).
...
| Dashboard | One `dashboardSnapshot()` pass; live poll now updates stats + department table, pauses when the tab is hidden, and is torn down on navigate. |
...
| Attendance | Employee history defaults to the current month. |
| JS | QR library is dynamically imported. Server time is session-cached 30s. Duplicate GETs are deduped. Page timers unsubscribe on `bacs:pagehide`. |
| Indexes | `app_notifications (user_id, created_at)` and `leaves (employee_id, status, start_date, end_date)`. Existing attendance unique + date/status indexes were already correct. |
...
Attendance rows, DTR calculations, and unread notification **content** stay live. Only directories, settings, the default schedule, and the calendar-events schema flag are cached.
...
2. On the admin dashboard, confirm `/admin/dashboard/live` updates the summary cards and department table, and that it stops when you switch modules.
